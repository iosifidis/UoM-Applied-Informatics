...
long encode(long code)
{
	...
	long x, y;
	y = (98 - (code*100) % 97) % 97; // Υπολογίζω τα ψηφία ελέγχου
	x = code * pow(10, (int)log10(y)+1) + y;
	/* Με log+1 υπολογίζω τον αριθμό των ψηφίων των ψηφίων ελέγχου.
	Μετά πολλαπλασιάζω το code με 10^αριθμό ψηφίωνν
	Τέλος προσθέτω σε αυτό που βρήκα τον αριθμό ψηφίων ελέγχου*/
	return x;

}
bool check(long ncode)
{
	if (encode(ncode)%97==1)
	{
	return TRUE;
	}
	else
