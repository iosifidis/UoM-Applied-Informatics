...
public class Registry {
	...
	ArrayList<Communication> allCommunications = new ArrayList<Communication>(); // Λίστα με όλες τις επικοινωνίες (τηλέφωνο/SMS)
	ArrayList<Suspect> allSuspects = new ArrayList<Suspect>(); //Λίστα με όλους τους υπόπτους
	...
	public void addCommunication(Communication aCommunication) {
		...
		for(int i=0;i<allSuspects.size();i++) {
			...
			if(allSuspects.get(i).phoneBook.contains(aCommunication.num1)) {
				...
				for(int j=0;j<allSuspects.size();j++) {
					...
					if(allSuspects.get(j).phoneBook.contains(aCommunication.num2)) {
						//Ενημέρωση της λίστας των συνεργατών
						allSuspects.get(j).possiblePartners.add(allSuspects.get(i)); 
						allSuspects.get(i).possiblePartners.add(allSuspects.get(j));
					}
				}
			}
		}
	...
	public Suspect getSuspectWithMostPartners() {
		...
		for(int i=0;i<allSuspects.size();i++)
			...
			for(int j=0;j<allSuspects.get(i).possiblePartners.size();j++) {
				//Έλεγχος με τον προσωρινό μέγα ύποπτο
				if(allSuspects.get(i).possiblePartners.size() > tempMax) {
						//Ανάθεση του νέου μέγα υπόπτου
						tempMax = allSuspects.get(i).possiblePartners.size();
						top=i;
